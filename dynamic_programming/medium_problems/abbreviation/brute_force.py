...
def brute_force_abbreviation(a: str, b: str) -> str:
    """
    Time Complexity: AverageCase(N + M)
    The optimizations I added
    here work well in the average case,
    but my guess is that when A is
    significantly larger than B, we
    have to traverse nearly the entire
    span of A SEVERAL times in the call
    stack while searching and that gets
    expensive fast. I'll need to break
    this up into a matrix of sub problems...
    """
...
class AbbreviationChecker:
    ...
    def search_for_abbreviation(self, a: str, index: int, recursion_level: int):
        log_recursion(recursion_level, index=index, a=a, b=self.b, continue_searching=self.continue_searching)
        if a == self.b:
...
if __name__ == '__main__':
    dynamically_generate_tests(functionality_test_data, brute_force_abbreviation, timed=True)
    run_dynamic_tests()
